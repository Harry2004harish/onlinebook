...
if(isset($_POST['add_product'])){
   ...
   $select_product_name = mysqli_query($conn, "SELECT name FROM `products` WHERE name = '$name' and type='featureproduct'") or die('query failed');

   if(mysqli_num_rows($select_product_name) > 0){
      $message[] = 'product name already added';
   }else{
      $add_product_query = mysqli_query($conn, "INSERT INTO `products`(name, price, image,product_desc,author,type) VALUES('$name', '$price', '$image','$product_desc','$author','featureproduct')") or die('query failed');
...
if(isset($_POST['update_product'])){
   ...
   mysqli_query($conn, "UPDATE `featuredproduct` SET name = '$update_name', price = '$update_price',product_desc='$update_product_desc',author='$update_author' WHERE id = '$update_p_id'") or die('query failed');
